...
if __name__ == '__main__':
    ...
    parser.add_argument('-y', '--yara-dir', type=str, help='Specify the yara rules directory (e.g. /full/path/to/yara/rules)')
    ...
    # Sanity check that the args exist and are directories
    if not os.path.isdir(args.yara_dir):
        print('--yara-dir directory not found.. should be /full/path/to/yara/rules..')
        sys.exit(1)
    ...
    print('Loading Yara Rules from {:s}'.format(args.yara_dir))
    my_rules = yara_rules.YaraRules(rule_path=args.yara_dir)
